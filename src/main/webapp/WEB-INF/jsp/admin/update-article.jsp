...
<title>修改文章 - java1024博客管理系统</title>
        ...
        <form action="/article/add" method="post" class="add-article-form">
          <div class="col-md-9">
            ...
            <div class="form-group">
              ...
              <input type="text" id="article-title" name="artTitle" class="form-control artTitle" placeholder="在此处输入标题" required autofocus autocomplete="off">
            </div>
            <!-- <div class="form-group">
              <label for="article-desc" class="sr-only">简介</label>
              <textarea id="article-desc" name="artDesc" class="form-control" placeholder="在此处输入简介" required autofocus autocomplete="off"> </textarea>
            </div> -->
            <div class="form-group">
              <label for="article-content" class="sr-only">内容</label>
              <!-- <script id="article-content" name="content" type="text/plain"></script> -->
              <textarea style="width:800px;height:400px;visibility:hidden;" id='artContent' name="artContent"></textarea>
            </div>
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
              	<textarea class="form-control" id="artDesc" name="artDesc" autocomplete="off"></textarea>
          ...
          <div class="col-md-3">
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
                  ...
                  <input type="hidden" id='artImage' name="artImage" /> 
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-footer">
                <button id="addArticleBtn" class="btn btn-primary" type="button" name="submit">发布</button>
              </div>
            </div>
          </div>
        </form>
...
<script id="${pageContext.request.contextPath }/uploadEditor" type="text/plain" style="display:none;"></script>
<script type="text/javascript">
$(function() {
	var content='';
		    		//alert("iiii1111");
	$.post(
		     "${pageContext.request.contextPath}/catagory/list",
		    function(data) {
		    	 content="";
		    	for(var i=0;i<data.length;i++) {
		           // if($('#category').val())
		            content+="<li> <label><input id='"+data[i].ctgId+"' name='artCtgId' type='radio' value='"+data[i].ctgId+"'>"
                    +data[i].ctgName+" <em class='hidden-md'>( 栏目ID: <span>"+data[i].ctgId+"</span> )</em></label></li>";
                    //alert(cateval);
					
		            //$("input:radio[value='"+cateval+"']").attr('checked','true');
		    	}
...
var cateval;
	...
	$(function(){
		//创建富文本编辑器
		itemAddEditor = E3.createEditor(".add-article-form [name=artContent]");
		//初始化类目选择和图片上传器
	...
	$.ajax({
        //几个参数需要注意一下
            ...
            url: "${pageContext.request.contextPath}/article/idjson?artId="+getUrlVars()["artId"] ,//url
            //data: "artId:"+getUrlVars()["artId"],
            success: function (data) {
         	  //alert(data.artId)
         	  $('.artTitle').val(data.artTitle);
         	  $('#artImage').val(data.artImage);
         	  $('#artId').val(data.artId);
         	  
         	 itemAddEditor.html(data.artContent);
         	 cateval=data.artCtgId;
         	 $('#artDesc').val(data.artDesc);
			$("#"+cateval).attr("checked","checked");
			//$("#artImage").attr("src",data.artImage);
         	 itemAddEditor.sync();
            },
            error : function() {
...
$('#addArticleBtn').click(function() {
	...
	$.ajax({
        //几个参数需要注意一下
            type: "POST",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "${pageContext.request.contextPath}/article/update" ,//url
